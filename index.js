...
function getYears(array, callbackFunction) {
    const years = callbackFunction(array).map((e) => {
        return e.Year
    })
    return years
}
...
function getWinners(array, callbackFunction) {
    const emptyArray = [];
    callbackFunction(array).forEach((e) => {
        if(e["Home Team Goals"] > e["Away Team Goals"]) {
            emptyArray.push(e["Home Team Name"])
        } else if (e["Home Team Goals"] < e["Away Team Goals"]) {
            emptyArray.push(e["Away Team Name"])
        }
    })
    return emptyArray
}
...
function getWinnersByYear(array, callbackFinals, callbackYears, callbackWinners) {
    const yearArray = callbackYears(array, callbackFinals)
    const winArray = callbackWinners(array, callbackFinals)
    const newArray = yearArray.map((item, index) => {
        return `In ${item}, ${winArray[index]} won the world cup!`
    })
    return newArray
}
